...
public class TelloVideoThread extends Thread {
    ...
    public TelloVideoThread(VideoListener listener) throws TelloNetworkException {

        this.queue = new TelloVideoQueue(listener);
        try {
    ...
    public void handleInput(byte[] bytes, int length) throws TelloException {
        if (!streamAligned) {
